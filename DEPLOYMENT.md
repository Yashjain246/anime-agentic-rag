...
> **Important — publish the OAuth app first.** While the Google Auth Platform
> publishing status is **"Testing"**, Google expires the refresh token after
> **7 days**, and the tool then silently returns `Calendar error: invalid_grant`
> on every request. Set the status to **In production** (Google Auth Platform →
> Audience → Publish app) *before* running `calendar_auth.py` — a token inherits
> the 7-day expiry from the status in effect when it was issued.
