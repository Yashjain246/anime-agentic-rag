...
def _submit_site_feedback(db, user_id: str, **kwargs) -> bool:
    """Wraps every DB write this widget makes so a failure here can never
    take down the page — this is an optional, best-effort action, and the
    DB layer's own _safe() wrapper already guards the query itself, but a
    second guard right at the UI call site means literally nothing this
    function does can propagate into a crash, regardless of the exact
    failure mode. Returns whether it actually succeeded, for the toast."""
    try:
        db.add_site_feedback(user_id, **kwargs)
        return True
    except Exception as e:
        logger.warning(f"Site feedback submit failed: {e}")
        return False


def _render_site_feedback() -> None:
    """One small overall-project feedback widget, not tied to any single
    reply — a plain up/down rating plus an optional comment tucked behind
    a popover so it never takes up page space until opened. Deliberately
    avoids emoji (△/▽ outline glyphs instead) to match the rest of the
    app's understated, professional visual language. Sized and positioned
    as a compact corner control, not a prominent section."""
    db = get_db()
    user_id = st.session_state.get("anon_user_id", "")

    cols = st.columns(3)
    with cols[0]:
        if st.button("△", key="site_fb_up", help="This project is helpful"):
            ok = _submit_site_feedback(db, user_id, rating="up")
            st.toast("Thanks for the feedback!" if ok else "Could not save feedback right now.")
    with cols[1]:
        if st.button("▽", key="site_fb_down", help="This project needs work"):
            ok = _submit_site_feedback(db, user_id, rating="down")
            st.toast("Thanks for the feedback!" if ok else "Could not save feedback right now.")
    with cols[2]:
        with st.popover("+", help="Leave a comment"):
            comment = st.text_area(
                "Comment",
                key="site_fb_comment_text",
                label_visibility="collapsed",
                placeholder="What would make this better?",
            )
            if st.button("Send", key="site_fb_comment_send"):
                if comment.strip():
                    ok = _submit_site_feedback(db, user_id, comment=comment.strip())
                    st.toast("Feedback sent — thank you!" if ok else "Could not save feedback right now.")
...
# Verified currently-airing (via anilist_schedule) as of writing — unlike
# the other pools, schedule questions age out the moment a show finishes
# its season, so this list needs the live tool checked again periodically
# rather than picking any well-known title from memory.
_SCHEDULE_POOL = [
    "When does the next episode of One Piece air?",
    "When does chainsmoker cat next episode air?",
    "When does Bleach: Thousand-Year Blood War next air?",
    "What time does Grand Blue Season 3 broadcast?",
]
_RATINGS_POOL = [
...
# ── Gentle anti-spam cooldown ─────────────────────────────────────────────────
# Not meant to slow down normal typing — a real user re-reading a reply and
# typing a follow-up never sends two messages within a couple seconds of each
# other. This only catches rapid-fire spam-clicking/scripted submissions,
# which otherwise burn through the shared Gemini/Tavily/OMDB quota for free.
_MIN_SECONDS_BETWEEN_MESSAGES = 2.5
if user_input:
    _now = time.monotonic()
    _last = st.session_state.get("_last_msg_ts", 0.0)
    if _now - _last < _MIN_SECONDS_BETWEEN_MESSAGES:
        st.toast("Sending a bit fast — give it a second, then try again.", icon="⏳")
        user_input = None
    else:
        st.session_state._last_msg_ts = _now
...
if user_input:
    ...
    with st.chat_message("user", avatar="app/assets/user_avatar.png"):
        st.markdown(
            f'<div class="user-bubble">{html.escape(user_input)}</div>',
            unsafe_allow_html=True,
        )
    ...
    with st.chat_message("assistant", avatar="app/assets/bot_avatar.png"):
        ...
        try:
            ...
            response_placeholder.markdown(
                badge + f'<div class="bot-bubble">{_linkify(escaped_reply)}</div>',
                unsafe_allow_html=True,
            )
...
# ─────────────────────────────────────────────────────────────────────────────
# OVERALL FEEDBACK — small corner widget, bottom-right of the main content
# ─────────────────────────────────────────────────────────────────────────────
st.divider()
_feedback_spacer, _feedback_widget = st.columns([5, 1])
with _feedback_widget:
    st.markdown(
        '<p style="text-align:right; color:#64748b; font-size:0.7rem; '
        'letter-spacing:0.05em; text-transform:uppercase; margin-bottom:0.2rem;">'
        "Feedback</p>",
        unsafe_allow_html=True,
    )
    _render_site_feedback()
