...
def _ensure_unzipped(zip_path: Path, extract_to: Path) -> None:
    ...
    logger.info(f"Extracting {zip_path.name} -> {extract_to} ...")
...
def get_lore_vectorstore() -> Chroma:
    ...
    if _lore_vectorstore is None:
        ...
        logger.info(f"[OK] Lore DB loaded: {_lore_vectorstore._collection.count()} chapters")
...
def get_recs_vectorstore() -> Chroma:
    ...
    if _recs_vectorstore is None:
        ...
        logger.info(f"[OK] Recs DB loaded: {_recs_vectorstore._collection.count()} anime")
