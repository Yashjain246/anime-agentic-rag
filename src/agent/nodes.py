...
def tools_node(state: AgentState) -> dict:
    ...
    system = SystemMessage(content=(
        ...
        "The user will often type an anime's name with a typo, mishearing, "
        "or joking nickname (e.g. \"chainsmoker cat\" for \"Chainsaw Man\"). "
        "Before calling anilist_schedule or trace_moe_vision, first work out "
        "what real anime they almost certainly mean and pass THAT correct, "
        "proper title to the tool - never pass the garbled text straight "
        "through. The tool does a literal text search with no fuzzy "
        "correction of its own, so an uncorrected title can silently match "
        "a completely unrelated, wrong show. If you are legitimately unsure "
        "what anime they mean (not just an obvious typo), ask them to "
        "clarify instead of guessing and calling the tool anyway.\n"
        "google_calendar_add requires human approval first - only call it "
        "when the user has explicitly asked to add something to their "
        "calendar, or is clearly agreeing to an offer YOU made earlier in "
        "RECENT CONVERSATION to add it (e.g. \"yes\", \"please add it\", "
        "\"sure, go ahead\"). A plain schedule question like \"when does X "
        "air?\" with no such request or agreement means: call "
        "anilist_schedule only, and do not call google_calendar_add - the "
        "user has to ask or agree first.\n"
        "Once that approval is present, always call anilist_schedule "
        "fresh, then google_calendar_add with the day/time it returns - do "
        "this even for a short confirmation reply where the anime was only "
        "named earlier in the conversation. Never reuse a time you see "
        "written in your own earlier reply in this conversation: those are "
        "already converted to IST for the user to read, while "
        "google_calendar_add needs the raw JST time from anilist_schedule's "
        "'Broadcast: ... JST' line - passing the IST display time to "
        "google_calendar_add creates an event at the wrong time.\n"
