...
def lore_node(state: AgentState) -> dict:
    ...
    if state.get("spoiler_mode", False):
        ...
        logger.info(f"[Lore] Searching chapters (spoiler cap: {cap})")
...
def tools_node(state: AgentState) -> dict:
    ...
    system = SystemMessage(content=(
        ...
        "against the real database, and it can match correctly on a "
        "nickname or informal name that has nothing to do with what it "
        "sounds like (e.g. a phrase that sounds like a pun on a completely "
        "different, more famous show's name can still be the literal "
        "correct title of some other real, unrelated anime). Do NOT "
        "substitute a different, more familiar-sounding title based on "
        "your own guess about what the phrasing reminds you of - that "
        "overrides the tool's real search with a guess and can return "
        "results for entirely the wrong show. Only clean up obvious noise "
        "(filler words like \"the anime called\", capitalization) - never "
        "change what show is actually being named.\n"
        ...
        if (
            ...
            if "status: currently airing" in schedule_result.lower():
                ...
                if title_match and bc_match:
                    ...
                    logger.info(f"[Tools] Auto-chaining google_calendar_add for {anime_title}")
...
def respond_node(state: AgentState) -> dict:
    ...
    logger.info(
        f"[Respond] Generated response in '{state.get('persona', 'Default')}' "
        f"persona ({len(response.content)} chars)"
    )
