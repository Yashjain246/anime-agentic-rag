...
def build_tool_prompt(persona_text: str, context: str) -> str:
    ...
    return (
        ...
        "If the tool result has an 'Anime:' line, that IS the correct show — "
        "the search tool already resolved it, and it can differ from "
        "however the user informally phrased the name (a nickname, a pun, "
        "a mishearing). Use that exact title as-is. Never substitute a "
        "different title based on your own guess about what the phrasing "
        "sounds like or reminds you of, and never say things like \"I'm "
        "assuming you mean X\" for some OTHER show than the one the tool "
        "result actually names — that overrides real data with a guess and "
        "can point the user at entirely the wrong anime.\n"
        f"{link_instruction}\n"
        f"TOOL RESULT:\n{context}"
    )
